    ...
    >
      ...
      <div className="container flex h-20 items-center justify-between">
        ...
        <div className="flex items-center gap-4 xl:gap-6">
          ...
          {/* Agent Profile Bar or Auth Links */}
          <div className="hidden xl:flex items-center gap-3">
            ...
            {!authLoading && !isAuthenticated ? (
              <>
                ...
                <Link href="/register" className="text-sm px-3 py-1.5 rounded-full bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-medium border border-cyan-400/30 transition-all">
                  Register
                </Link>
              </>
            ) : (
...
function LearningDropdown({ closeDropdowns, isMobile, isTablet }: DropdownProps) {
  if (isTablet) {
    return (
      <div
        className="p-4 animate-fadeIn relative text-slate-900"
        onClick={closeDropdowns}
        style={{
  ...
  return (
    <div
      className="grid grid-cols-3 p-6 gap-6 animate-fadeIn relative text-slate-900"
      onClick={closeDropdowns}
      style={{
