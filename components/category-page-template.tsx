        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {relatedCategories.map((category) => (
              <div key={category.path} className="w-full flex items-center justify-between bg-gray-100 rounded-lg p-4 hover:bg-yellow-500">
                <span className="text-gray-800">{category.title}</span>
                <StartCourseButton path={category.path} className="ml-2" />
              </div>
            ))}
          </div>
        </div>
